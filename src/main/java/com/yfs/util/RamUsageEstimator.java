...
import com.yfs.constant.SystemProperties;
...
public final class RamUsageEstimator {
    ...
    public static enum JvmFeature {
        ...
        /**
         * Array header size estimated using array based offset
         */
        ARRAY_HEADER_SIZE("Array header size estimated using array based offset"),
        /**
         * Shallow instance size based on field offsets
         */
    ...
    static {
        ...
        objectHeader = SystemProperties.JRE_IS_64BIT ? (8 + referenceSize) : 8;
        arrayHeader = SystemProperties.JRE_IS_64BIT ? (8 + 2 * referenceSize) : 12;
        ...
        try {
            ...
            final int ofs1 = ((Number) objectFieldOffsetM.invoke(THE_UNSAFE, dummy1Field)).intValue();
            final Field dummy2Field = DummyTwoLongObject.class.getDeclaredField("dummy2");
            final int ofs2 = ((Number) objectFieldOffsetM.invoke(THE_UNSAFE, dummy2Field)).intValue();
        ...
        try {
            final Class<? extends PlatformManagedObject> beanClazz = Class.forName("com.sun.management.HotSpotDiagnosticMXBean").asSubclass(PlatformManagedObject.class);
        ...
        JVM_INFO_STRING = "[JVM: " + SystemProperties.JVM_NAME + ", " + SystemProperties.JVM_VERSION + ", " + SystemProperties.JVM_VENDOR + ", " + SystemProperties.JAVA_VENDOR + ", " + SystemProperties.JAVA_VERSION + "]";
    }
            ...
            if (obClazz.isArray()) {
                ...
                if (len > 0) {
                    ...
                    if (componentClazz.isPrimitive()) {
                        size += (long) len * PRIMITIVE_SIZES.get(componentClazz);
                    } else {
                        size += (long) NUM_BYTES_OBJECT_REF * len;
    ...
    private static long adjustForField(long sizeSoFar, final Field f) {
        ...
        if (OBJECT_FIELD_OFFSET_METHOD != null) {
            try {
                final long offsetPlusSize = ((Number) OBJECT_FIELD_OFFSET_METHOD.invoke(THE_UNSAFE, f)).longValue() + fsize;
    ...
    static final class IdentityHashSet<KType> implements Iterable<KType> {
        ...
        public static final int INT_16 = 0x80000000;
        ...
        protected int nextCapacity(int current) {
            ...
            if (current < MIN_CAPACITY / TWO) {
                current = MIN_CAPACITY / TWO;
            }
            return current << 1;
        }
