...
public class Randoms {
    ...
    public static String randomCodes(int iLength, boolean isCapital) {
        ...
        int[] iCodes = random(0, 35, iLength, true); // 因为是26个字母，10-35是26个数字
    ...
    public static String randomLetter(int iLength, int type) {
        Validate.isTrue(iLength > 0);
        int[] iCodes = random(0, 25, iLength, true); // 26个字母，0-25是26个数字
        StringBuffer strb = new StringBuffer();
        for (int i = 0; i < iCodes.length; i++) {
            char cTmp = ' ';
            if (type == 1) {
    ...
    /** 从集合中随机取出若干个元素 */
    public static <T> List<T> randomCollection(List<T> source, int size) {
