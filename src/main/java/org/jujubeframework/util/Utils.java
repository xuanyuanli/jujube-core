...
public class Utils {
    ...
    public static File appendStringToFile(String fileName, String data, Charset encoding) {
        ...
        try {
            Files.asCharSink(file, encoding, FileWriteMode.APPEND).write("\r\n" + data);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        return file;
    ...
    public static String asyncKey() {
        return Thread.currentThread().getId() + ":" + Randoms.randomCodes(5) + Dates.formatNow("-yyyyMMdd:HHmmss");
    }
    ...
    public static Map<String, Object> newHashMap(Object... keyValue) {
        int size = keyValue != null ? keyValue.length / 2 : 0;
        Map<String, Object> map = new HashMap<String, Object>(size);
        if (keyValue == null || keyValue.length == 0) {
            return map;
        }
        int two = 2;
        if (keyValue.length % two != 0) {
            throw new IllegalArgumentException();
        }
        for (int i = 0; i < keyValue.length; i += two) {
